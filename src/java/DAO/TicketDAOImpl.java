...
public void insert(Ticket ticket) throws Exception {
    String sql = "INSERT INTO ticket (reservationNumber, travelDate, ticketStatus, origin, destination, stock, departureTime, arrivalTime, basePrice, ticketType, facilities) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    try (PreparedStatement ps = con.prepareStatement(sql)) {
        ...
        ps.setString(10, ticket.getTicketType());

        // Jika tiket eksekutif, set facilities, jika tidak set null
        if (ticket instanceof Executive) {
            ps.setString(11, ((Executive) ticket).getFacilities());
        } else {
            ps.setString(11, null);  // Tiket Regular tidak punya fasilitas
        }
