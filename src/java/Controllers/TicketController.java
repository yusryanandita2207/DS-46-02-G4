   ...
   if ("add".equals(action)) {
    try {
        ...
        if (!departureTime.matches("\\d{2}:\\d{2}:\\d{2}")) {
            departureTime += ":00";  // Menambahkan :00 jika format tidak lengkap
        }
        
        if (!arrivalTime.matches("\\d{2}:\\d{2}:\\d{2}")) {
            arrivalTime += ":00";  // Menambahkan :00 jika format tidak lengkap
        }
        
//        Ticket ticketModel;
        if ("Executive".equals(ticketType)) {
            String facilities = request.getParameter("facilities");  // Dapatkan fasilitas hanya jika Executive
        ...
        e.printStackTrace();  // Tampilkan error di console untuk debugging
